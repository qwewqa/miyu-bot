...
class DetailTabButton(discord.ui.Button["FilterDetailView"]):
    def __init__(self, tab: int, style=discord.ButtonStyle.primary, **kwargs):
    ...
    async def callback(self, interaction: Interaction):
        ...
        await interaction.response.edit_message(
            embed=self.view.active_embed, view=self.view
        )
...
class DetailServerShortcutButton(discord.ui.Button["FilterDetailView"]):
    def __init__(self, server: Server, style=discord.ButtonStyle.secondary, **kwargs):
        self.server = server
        emoji = SERVER_EMOJI[server]
        super(DetailServerShortcutButton, self).__init__(
            style=style, emoji=emoji, **kwargs
        )
...
class DetailCommandChangeButton(discord.ui.Button["FilterDetailView"]):
    def __init__(
        ...
        **kwargs,
        ...
        super(DetailCommandChangeButton, self).__init__(
            style=style, emoji=emoji, **kwargs
        )
    ...
    async def callback(self, interaction: Interaction):
        ...
        new_source = command_sources[
            (command_sources.index(self.view.source_info) + 1) % len(command_sources)
        ]
        view = FilterDetailView(
            self.view.master_filter,
            self.view.ctx,
            self.view.base_results,
...
class DetailToListButton(discord.ui.Button["FilterDetailView"]):
    ...
    async def callback(self, interaction: Interaction):
        assert self.view is not None
        view = filter_list_view.FilterListView(
...
class FilterDetailView(UserRestrictedView):
    def __init__(
        ...
        **kwargs,
        ...
        source_info = (
            source_info
            or results.command_source_info
            or master_filter.command_sources[0]
        )
        ...
        self.embed_source = (
            ...
            else self.wrap_tabless_source(source_info.embed_source)
        )
        ...
        if source_info.tabs is not None:
            ...
            self._tab = None
        self.active_embed = self.embed_source(
            master_filter,
            ctx,
        ...
        self.prev_button = PageChangeButton(
            -1,
            log_name="filter_detail_page_change",
            emoji="<:prev:860683672382603294>",
            row=1,
        )
        self.next_button = PageChangeButton(
            1,
            log_name="filter_detail_page_change",
            emoji="<:next:860683672402526238>",
            row=1,
        )
        ...
        self.large_decr_button = PageChangeButton(
            -20, log_name="filter_detail_page_change_extra", label="-20", row=2
        )
        self.small_decr_button = PageChangeButton(
            -5, log_name="filter_detail_page_change_extra", label="-5", row=2
        )
        self.small_incr_button = PageChangeButton(
            5, log_name="filter_detail_page_change_extra", label="+5", row=2
        )
        self.large_incr_button = PageChangeButton(
            20, log_name="filter_detail_page_change_extra", label="+20", row=2
        )
        self.page_display_button = Button(
            disabled=True, style=discord.ButtonStyle.secondary, row=2
        )
        ...
        for shortcut in source_info.shortcut_buttons:
            button = ShortcutButton(
                ...
                label=master_filter.l10n[ctx.preferences.language].format_value(
                    shortcut.name
                ),
    ...
    def server_available(self, server):
        return (
            self.master_filter.get_by_id(self.active_value.id, self.ctx, server)
            is not None
        )
    ...
    def update(self):
        ...
        self.active_embed = self.embed_source(
            self.master_filter, self.ctx, value, self.tab, self.server
        )
        ...
        self.page_display_button.label = (
            f"{self._page_index + 1}/{self.max_page_index + 1}"
        )
