...
from miyu_bot.commands.common.deletable_message import (
    DeleteButton,
...
async def run_paged_message(
    ...
    *,
    ...
    page_titles = [
        f"Page {i + 1}. #{i * page_size + 1}-{i * page_size + len(page)}"
        for i, page in enumerate(paged_entries)
    ]

    await ctx.send(
        embed=embeds[start_page],
        files=files,
        view=PagedMessageView(
            embeds,
...
class PageChangeButton(discord.ui.Button["PagedMessageView"]):
    def __init__(
        ...
        log_name: Optional[str] = "paged_page_change_button",
        **kwargs,
    ):
...
class PagedMessageView(UserRestrictedView):
    def __init__(
        ...
        **kwargs,
        ...
        self.first_button = PageChangeButton(
            -self.max_page_index, emoji="<:first:860679908258873344>", row=0
        )
        self.prev_button = PageChangeButton(
            -1, emoji="<:prev:860683672382603294>", row=0
        )
        self.next_button = PageChangeButton(
            1, emoji="<:next:860683672402526238>", row=0
        )
        self.last_button = PageChangeButton(
            self.max_page_index, emoji="<:last:860679908426514465>", row=0
        )
        ...
        self.page_select = PageSelect(
            placeholder=self.get_select_placeholder(start_index),
            options=self.get_select_options(),
            row=1,
        )
        if len(embeds) > 1:
            self.add_item(self.page_select)
        self.prev_page_select = PageSelectPageChangeButton(
            -1, emoji="<:prev:860683672382603294>", row=2
        )
        self.next_page_select = PageSelectPageChangeButton(
            1, emoji="<:next:860683672402526238>", row=2
        )
    ...
    def select_page_index(self, value):
        ...
        self.page_select.placeholder = self.get_select_placeholder(
            self.select_page_index
        )
        self.prev_page_select.disabled = self.select_page_index == 0
        self.next_page_select.disabled = (
            self.select_page_index == self.max_select_page_index
        )
    ...
    def get_select_option_label(self, page_index: int):
        return (
            self.page_titles[page_index] if self.page_titles else f"{page_index + 1}."
        )

    def get_select_options(self):
        return [
            SelectOption(label=self.get_select_option_label(i), value=str(i))
            for i in range(
                self.select_page_index * 25,
