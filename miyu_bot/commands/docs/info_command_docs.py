...
from miyu_bot.commands.docs.documentation_fluent_localization import (
    DocumentationFluentLocalization,
)
...
def generate_info_command_docs(
    bot: MiyuBot, docs_path: Path, filters: List[MasterFilter]
):
    for master_filter in filters:
        generate_master_filter_docs(bot, docs_path, master_filter)


def generate_master_filter_docs(
    bot: MiyuBot, docs_path: Path, master_filter: MasterFilter
):
    for locale in valid_locales:
        ...
        md.add_all(
            get_attribute_help_text(attr, l10n)
            for attr in master_filter.data_attributes
        )
        filename = master_filter.name.replace("_filter", "").replace("_", "-")
        (docs_path / f"{filename}.{locale_filename}.md").write_text(
            md.get(), encoding="utf-8"
        )


def get_command_source_help_texts(
    master_filter: MasterFilter, l10n: DocumentationFluentLocalization, heading_level=3
    ...
    for command in master_filter.command_sources:
        if command.command_args:
            ...
            md.heading(
                heading_level,
                f"{name}" + (f" ({localized_name})" if name != localized_name else ""),
            )
            md.text(
                f'*[{l10n.format_value("command-type-detail")}]({USAGE_PAGE}#detail-commands)*'
            )
            if command.suffix_tab_aliases:
                md.admonition(
                    "note",
                    l10n.format_value("command-tab-names"),
                    ", ".join(command.suffix_tab_aliases),
                )
            if desc := l10n.format_value(
                f'command-{command.command_args["name"]}-description', fallback=""
            ):
    ...
    if master_filter.list_formatter:
        ...
        md.text(
            f'*[{l10n.format_value("command-type-list")}]({USAGE_PAGE}#list-commands)*'
        )
        if desc := l10n.format_value(
            f'command-{master_filter.list_formatter.command_args["name"]}-description',
            fallback="",
        ):
            md.admonition("question", "Description", desc)
        yield md
...
def get_attribute_help_text(
    attr: DataAttributeInfo, l10n: DocumentationFluentLocalization, heading_level=3
) -> MarkdownDocument:
    md = MarkdownDocument()
    name = attr.name
    ...
    md.heading(
        heading_level,
        attr.name + (f" ({localized_name})" if name != localized_name else ""),
    )
    if attr.aliases:
    ...
    if attr.value_mapping:
        ...
        md.admonition(
            "note",
            "Tags",
            md.escape_markdown("\n".join(tag_group_lines)),
            collapsible=True,
        )
    elif attr.is_flag:
        md.admonition(
            "note", "Tags", md.escape_markdown(", ".join([attr.name, *attr.aliases]))
        )
...
def get_attribute_usage(
    attr: DataAttributeInfo, l10n: DocumentationFluentLocalization
) -> str:
    entries = []
    if attr.is_flag:
...
def get_attribute_type_description(
    attr: DataAttributeInfo, l10n: DocumentationFluentLocalization
):
    attr_types = []
    if attr.is_flag:
        if attr.flag_callback:
            attr_types.append(
                f'[{l10n.format_value("attr-type-special-flag")}]({USAGE_PAGE}#flag)'
            )
        else:
            attr_types.append(
                f'[{l10n.format_value("attr-type-flag")}]({USAGE_PAGE}#flag)'
            )
    if attr.is_sortable:
        if attr.is_default_sort:
            attr_types.append(
                f'[{l10n.format_value("attr-type-sortable-default")}]({USAGE_PAGE}#sortable)'
            )
        else:
            attr_types.append(
                f'[{l10n.format_value("attr-type-sortable")}]({USAGE_PAGE}#sortable)'
            )
    if attr.formatter:
        if attr.is_default_display:
            attr_types.append(
                f'[{l10n.format_value("attr-type-display-default")}]({USAGE_PAGE}#display)'
            )
        else:
            attr_types.append(
                f'[{l10n.format_value("attr-type-display")}]({USAGE_PAGE}#display)'
            )
    if attr.is_comparable:
        attr_types.append(
            f'[{l10n.format_value("attr-type-comparable")}]({USAGE_PAGE}#comparable)'
        )
    if attr.is_eq:
        attr_types.append(
            f'[{l10n.format_value("attr-type-equality")}]({USAGE_PAGE}#equality)'
        )
    if attr.is_tag:
        attr_types.append(f'[{l10n.format_value("attr-type-tag")}]({USAGE_PAGE}#tag)')
    if attr.is_keyword:
        attr_types.append(
            f'[{l10n.format_value("attr-type-keyword")}]({USAGE_PAGE}#keyword)'
        )
    if attr.is_plural:
        attr_types.append(
            f'[{l10n.format_value("attr-type-plural")}]({USAGE_PAGE}#plural)'
        )
    return attr_types
