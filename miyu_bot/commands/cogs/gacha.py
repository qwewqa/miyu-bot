...
class Gacha(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        self.unit_icons = {
            ...
            7: self.load_image("CharaIcon_DjUnitIcon_COA.png"),
            8: self.load_image("CharaIcon_DjUnitIcon_UNI.png"),
            9: self.load_image("CharaIcon_DjUnitIcon_AM.png"),
            10: self.load_image("CharaIcon_DjUnitIcon_ABCD.png"),
    ...
    async def create_card_image_grid_async(
        self, cards: List[CardMaster], row_size: int = 16
    ):
    ...
    def create_card_image_grid(self, cards: List[CardMaster], row_size: int = 20):
        ...
        img = Image.new(
            "RGBA", (259 * row_size, max(1, 259 * column_count)), (255, 255, 255, 0)
        )
    ...
    def _get_card_icon(self, card_id: int):
        ...
        if card.rarity_id >= 2:
            img.paste(
                self.rarity_frames[card.rarity_id],
                (0, 0),
                self.rarity_frames[card.rarity_id],
            )
        img.paste(
            self.unit_icons[card.character.unit_id],
            (2, 2),
            self.unit_icons[card.character.unit_id],
        )
        img.paste(
            self.attribute_icons[card.attribute_id],
            (194, 2),
            self.attribute_icons[card.attribute_id],
        )
    ...
    async def pull(self, ctx: PrefContext, *, arg: Optional[ParsedArguments]):
        ...
        view, embed, file = await self.do_gacha_draw_and_get_message_data(
            ctx.author, gacha, draw_data, ctx.assets, ctx.preferences.server
        )
    ...
    async def send_banner_pulls_message(
        self, ctx: PrefContext, user_id: int, gacha_id: int
    ):
    ...
    async def do_gacha_draw_and_get_message_data(
        self,
        ...
        img = await self.create_pull_image(
            draw_result.cards, draw_result.bonus, draw_result.sub_bonus
        )
        ...
        view = GachaPullView(
            self, GachaPullInvokeData(user, gacha, draw_data, assets, server)
        )

        return view, embed, discord.File(fp=buffer, filename="pull.png")

    async def do_gacha_draw(
        self,
        user: Union[discord.User, discord.Member],
        gacha: GachaMaster,
        draw_data: GachaDrawMaster,
        ...
        async with self.pull_locks[user.id], in_transaction() as conn:
            tables = gacha.tables
            tables_rates = [
                list(itertools.accumulate(t.rate for t in table)) for table in tables
            ]

            state, _ = await GachaState.get_or_create(
                user_id=user.id, server_id=int(server), gacha_id=gacha.id, using_db=conn
            )
            state.total_roll_counter += 1

            cards = []
            for draw_amount, table_rate in zip(
                draw_data.draw_amounts, gacha.table_rates
            ):
                ...
                for _i in range(draw_amount):
                    ...
                    result_index = next(
                        i for i, s in enumerate(table_rates) if rng <= s
                    )
                    card = assets.card_master[tables[table_index][result_index].card_id]
                    await self.register_card_pulled(
                        user.id,
                        int(server),
                        gacha.id,
                        table_rate.id,
            ...
            if gacha.bonus:
                ...
                if (
                    gacha.sub_bonus
                    and prev_pity < gacha.sub_bonus.max_value
                    and current_pity >= gacha.sub_bonus.max_value
                ):
                    state.total_counter += 1
                    ...
                    table_rates = list(
                        itertools.accumulate(
                            t.rate for t in sub_bonus_tables[table_index]
                        )
                    )
                    rng = random.randint(1, table_rates[-1])
                    result_index = next(
                    ...
                    sub_bonus = assets.card_master[
                        sub_bonus.tables[table_index][result_index].card_id
                    ]
                    await self.register_card_pulled(
                        user.id,
                        int(server),
                        gacha.id,
                        gacha.sub_bonus.table_rate.id,
                        sub_bonus.id,
                        state.total_counter,
                        state.total_roll_counter,
                        conn,
                ...
                if gacha.bonus and current_pity >= gacha.bonus.max_value:
                    ...
                    table_index = next(i for i, s in enumerate(rates) if rng <= s)
                    table_rates = list(
                        itertools.accumulate(t.rate for t in bonus_tables[table_index])
                    )
                    rng = random.randint(1, table_rates[-1])
                    result_index = next(
                        i for i, s in enumerate(table_rates) if rng <= s
                    )
                    bonus = assets.card_master[
                        bonus_tables[table_index][result_index].card_id
                    ]
                    await self.register_card_pulled(
                        user.id,
                        int(server),
                        gacha.id,
                        gacha.bonus.table_rate.id,
                        bonus.id,
                        state.total_counter,
                        state.total_roll_counter,
                        conn,
    ...
    async def register_card_pulled(
        self,
        user_id: int,
        server_id: int,
        gacha_id: int,
        ...
        entry, _created = await CollectionEntry.get_or_create(
            user_id=user_id,
            server_id=server_id,
            gacha_id=gacha_id,
...
class GachaPullAgainButton(discord.ui.Button["GachaPullView"]):
    async def callback(self, interaction: discord.Interaction):
        ...
        view, embed, file = await self.view.cog.do_gacha_draw_and_get_message_data(
            *invoke_data
        )
...
class GachaPullView(discord.ui.View):
    def __init__(self, cog: Gacha, data: GachaPullInvokeData, *args, **kwargs):
        ...
        self.add_item(
            GachaPullAgainButton(label="Pull Again", style=discord.ButtonStyle.success)
        )
