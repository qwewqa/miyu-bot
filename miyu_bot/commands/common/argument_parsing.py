...
def _parse_named_argument(arg):
    ...
    operator = groups[2]
    values = [
        value[1:-1] if _param_string_re.fullmatch(value) else value
        for value in _param_argument_re.findall(groups[3])
    ]
...
def parse_arguments(arg):
    ...
    original = arg
    named_arguments_parsed = [
        _parse_named_argument(na[0]) for na in _param_re.findall(arg)
    ]
...
class ParsedArguments(discord.ext.commands.Converter):
    ...
    def text(self):
        return " ".join(
            word
            for word in self.text_argument.split()
            if word.lower() not in self.used_words
        )
    ...
    def single(
        ...
        is_list=False,
        numeric=False,
        converter: Union[dict, Callable] = lambda n: n,
    ):
    ...
    def single_op(
        ...
        value = [
            arg
            for args in (self.named_arguments.get(name) for name in names)
            if args
            for arg in args
        ]
        if not value:
        ...
        if numeric:
            try:
                ...
                raise ArgumentError(
                    f'Expected numerical arguments for parameter "{name}".'
                )
        try:
            if isinstance(converter, dict):
                value = ArgumentValue(
                    [converter[v.lower()] for v in value.value], value.operator
                )
            else:
                value = ArgumentValue(
                    [converter(v) for v in value.value], value.operator
                )
        except Exception:
    ...
    def repeatable_op(
        ...
        if numeric:
            try:
                values = [
                    ArgumentValue([float(v) for v in value.value], value.operator)
                    for value in values
                ]
            except ValueError:
                raise ArgumentError(
        ...
        try:
            if isinstance(converter, dict):
                ...
                values = [
                    ArgumentValue([converter(v) for v in value.value], value.operator)
                    for value in values
                ]
        except Exception:
            raise ArgumentError(f'Invalid value for parameter "{name}".')
    ...
    def has_unused_named_arguments(self):
        return any(
            name not in self.used_named_arguments
            for name in self.named_arguments.keys()
        )
        ...
        for name in preference_values.keys():
            ...
            if override:
                ...
                if error_message := preference.validate_or_get_error_message(override):
                    raise ArgumentError(
                        f'Invalid value "{override}" for preference "{name}": {error_message}'
                    )
                preference_values[name] = preference.convert(
                    preference.transform(override)
                )
...
_list_operators = {
    ...
    "==": lambda a, b: all(
        a == v for v in b
    ),  # Not very useful when b has more than one element
...
_list_to_list_operators = {
    ...
    ">": lambda a, b: all(
        u > v for v in b for u in a
    ),  # Also not very useful when b has more than one element
